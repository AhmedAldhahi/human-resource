...
export default function WageProcessingModal({ isOpen, onClose, draft, month, onSuccess }: WageProcessingModalProps) {
  ...
  useEffect(() => {
    if (isOpen && draft) {
      ...
      } else if (draft.activeAdvanceDeduction && draft.activeAdvanceDeduction > 0) {
        setDeductions([{ amount: draft.activeAdvanceDeduction, notes: draft.activeAdvanceNote || (isRtl ? 'خصم قسط سلفة الراتب' : 'Salary advance installment') }]);
      } else {
  ...
  const handleSave = async (status: PayrollStatus) => {
    ...
    const bonusNotesStr = bonuses.filter(b => b.amount > 0 || b.notes).map(b => `${b.notes || (isRtl ? 'مكافأة' : 'Bonus')} (${b.amount} JOD)`).join(' | ');
    const deductionNotesStr = deductions.filter(d => d.amount > 0 || d.notes).map(d => `${d.notes || (isRtl ? 'خصم' : 'Deduction')} (${d.amount} JOD)`).join(' | ');
    ...
    try {
      ...
      setError(err?.response?.data?.message || (isRtl ? 'فشل حفظ سجل الراتب' : 'Failed to save payroll record'));
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="relative w-full max-w-2xl bg-slate-900 border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="p-6 border-b border-white/10 shrink-0">
          <h2 className="text-xl font-extrabold text-white">
            {isRtl ? `معالجة أجر الموظف: ${draft.name}` : `Process Wage: ${draft.name}`}
          </h2>
          <p className="text-slate-400 text-sm mt-1">
            {isRtl ? `الشهر: ${month} | الهيكل: ${isFixed ? 'راتب ثابت' : 'أجر بالساعة'}` : `Month: ${month} | Type: ${draft.employeeType}`}
          </p>
        </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 rounded-xl bg-slate-800/50 border border-white/5 space-y-1">
              <label className="text-xs font-bold text-slate-400 uppercase">{isRtl ? 'الساعات المسجلة' : 'Tracked Hours'}</label>
              <p className="text-xl font-bold text-white" dir="ltr">{draft.trackedHours}h</p>
            </div>
            <div className="p-4 rounded-xl bg-slate-800/50 border border-white/5 space-y-1">
              <label className="text-xs font-bold text-slate-400 uppercase">{isRtl ? 'الساعات المعتمدة' : 'Approved Hours'}</label>
              <input
                type="number"
                step="0.5"
                min="0"
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-bold text-emerald-400 border-b border-emerald-500/20 pb-2">{isRtl ? 'المستحقات والإضافات' : 'Earnings'}</h3>
            <div className="flex justify-between text-sm">
              <span className="text-slate-400">{isRtl ? `الأجر الأساسي ${isFixed ? '(ثابت)' : '(بالساعة)'}` : `Base Wage ${isFixed ? '(Fixed)' : '(Hourly)'}`}</span>
              <span className="text-white font-bold" dir="ltr">{baseWage.toFixed(2)} JOD</span>
            </div>
            {draft.transportationAllowance > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">{isRtl ? 'بدل التنقلات' : 'Transportation Allowance'}</span>
                <span className="text-white font-bold" dir="ltr">+{draft.transportationAllowance.toFixed(2)} JOD</span>
              </div>
            )}
            {draft.recurringBonus > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">{isRtl ? 'المكافأة الشهرية الثابتة' : 'Recurring Bonus'}</span>
                <span className="text-white font-bold" dir="ltr">+{draft.recurringBonus.toFixed(2)} JOD</span>
              </div>
            )}
            {pointsJOD > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-slate-400">{isRtl ? 'مكافأة صافي نقاط البطاقات' : 'Net Card Points Bonus (Ref)'}</span>
                <span className="text-emerald-400 font-bold" dir="ltr">+{pointsJOD.toFixed(2)} JOD</span>
              </div>
            )}
          </div>

          <div className="space-y-3">
            <h3 className="text-sm font-bold text-red-400 border-b border-red-500/20 pb-2">{isRtl ? 'الخصومات والاستقطاعات' : 'Deductions'}</h3>
            {draft.transportationDeductions > 0 && (
              <div className="flex justify-between text-sm text-red-400">
                <span>{isRtl ? `خصم بدل التنقل للعمل من المنزل (${draft.wfhDays} أيام)` : `WFH Transportation Deduction (${draft.wfhDays} days)`}</span>
                <span className="font-bold" dir="ltr">-{draft.transportationDeductions.toFixed(2)} JOD</span>
              </div>
            )}
            {draft.activeAdvanceDeduction !== undefined && draft.activeAdvanceDeduction > 0 && (
              <div className="flex justify-between text-sm text-amber-300 font-bold bg-amber-500/10 p-2.5 rounded-xl border border-amber-500/20">
                <span>💳 {isRtl ? 'خصم قسط سلفة الراتب الشهرية (تلقائي)' : 'Active Salary Advance Installment (Auto)'}</span>
                <span className="font-mono" dir="ltr">-{draft.activeAdvanceDeduction.toFixed(2)} JOD</span>
              </div>
            )}
            {pointsJOD < 0 && (
              <div className="flex justify-between text-sm text-red-400">
                <span>{isRtl ? 'خصم صافي نقاط البطاقات' : 'Net Card Points Penalty (Ref)'}</span>
                <span className="font-bold" dir="ltr">{pointsJOD.toFixed(2)} JOD</span>
              </div>
            )}
          </div>

          <div className="space-y-4 pt-4 border-t border-white/10">
            <h3 className="text-sm font-bold text-indigo-400">{isRtl ? 'التعديلات والمنح اليدوية' : 'Manual Adjustments'}</h3>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-bold text-emerald-400">{isRtl ? 'المكافآت اليدوية (دينار)' : 'Bonuses (JOD)'}</label>
                  <button onClick={() => setBonuses([...bonuses, {amount: 0, notes: ''}])} className="text-xs font-bold text-emerald-400/80 hover:text-emerald-300 bg-emerald-500/10 px-2 py-1 rounded">{isRtl ? '+ إضافة' : '+ Add'}</button>
                </div>
                {bonuses.map((b, idx) => (
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-bold text-red-400">{isRtl ? 'الخصومات اليدوية (دينار)' : 'Deductions (JOD)'}</label>
                  <button onClick={() => setDeductions([...deductions, {amount: 0, notes: ''}])} className="text-xs font-bold text-red-400/80 hover:text-red-300 bg-red-500/10 px-2 py-1 rounded">{isRtl ? '+ إضافة' : '+ Add'}</button>
                </div>
        ...
        <div className="p-6 border-t border-white/10 bg-slate-900/80 shrink-0 flex flex-col sm:flex-row justify-between items-center gap-4">
          <div className={`${isRtl ? 'text-right' : 'text-left'}`}>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{isRtl ? 'صافي الراتب النهائي المستحق' : 'Final Net Payout'}</p>
            <p className="text-3xl font-black text-emerald-400" dir="ltr">{finalPayout.toFixed(2)} JOD</p>
          </div>
          <div className="flex gap-3">
            ...
            >
              {loading ? (isRtl ? 'جاري الحفظ...' : 'Saving...') : (isRtl ? 'اعتماد الراتب الصافي' : 'Finalize Wage')}
            </button>
          </div>
        </div>
      </div>
